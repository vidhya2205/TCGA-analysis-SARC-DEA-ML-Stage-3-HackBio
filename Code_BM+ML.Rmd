...
# Step 2: Selecting top genes based on variability (standard deviation)
SD = apply(trans, 2, sd)
...
ggplot(selectResults.levels, aes(x = logFC, y = -log10(FDR))) +
    geom_point(aes(color = diff_exp, shape = Highlight), alpha = 0.8, size = 2) + # for DGEA analysis
    geom_point(data = subset(selectResults.levels, Highlight == "In ML"), 
               aes(x = logFC, y = -log10(FDR), shape = "In ML"), 
               size = 3, stroke = 1.2) + # Show legend for ML feature points
